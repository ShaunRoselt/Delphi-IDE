...
export function renderRuntime() {
  ...
  const winClass = `runtime-floatwin${minimized ? ' minimized' : ''}`

  return `
    <div class="runtime-backdrop"></div>
    <div class="${winClass}" data-floatwin style="left:${w.x}px;top:${w.y}px;width:${w.width}px;height:${minimized ? 31 : w.height + 31}px">
      <header class="runtime-floatwin-caption" data-floatwin-drag>
        <span class="rt-form-icon">◈</span>
        <span class="runtime-title">${escapeHtml(r.caption)}</span>
        <div class="rt-caption-btns">
          ...
          <button type="button" class="rt-close-btn" title="Stop &amp; Close" data-floatwin-action="stop">×</button>
        </div>
      </header>
      ${minimized ? '' : `
